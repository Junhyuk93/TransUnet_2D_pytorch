...
class DentalDataset(Dataset):
    ...
    def __init__(self, path, transform):
        ...
        self.ids =sorted(os.listdir(img_folder))
        
        for patient_dir in self.ids:
            patient_images_dir = os.path.join(img_folder, patient_dir)
            patient_masks_dir = os.path.join(mask_folder, patient_dir)
            patient_image_files = sorted(os.listdir(patient_images_dir))
            
            for image_file in patient_image_files:
                self.img_paths.append(os.path.join(patient_images_dir, image_file))
                self.mask_paths.append(os.path.join(patient_masks_dir, image_file))
        
        # for p in os.listdir(img_folder):
        #     #name = p.split('.')[0]
        #     name = p
        #     self.img_paths.append(os.path.join(img_folder, name))
        #     self.mask_paths.append(os.path.join(mask_folder, name))
